...
% update may 2013  , added retrieval of block date and recording time
...
if H.ConnectServer('local','me') == 0
    disp('ERROR: Connecting to server failed');
    close(F)
    return
end

if H.OpenTank(EVENT.Mytank, 'R') == 0
    disp('ERROR: Opening tank failed');
    H.ReleaseServer;
    close(F)
    return
end    

if H.SelectBlock(EVENT.Myblock) == 0
    disp('ERROR: Opening block failed');
    H.CloseTank;
    H.ReleaseServer;
    close(F)
    return
...
%get block start and stop times. 
starttime = H.CurBlockStartTime();
stoptime = H.CurBlockStopTime();
EVENT.BlockStrTimes = [ H.FancyTime(starttime ,'D/O/Y H:M:S') ' - ' H.FancyTime(stoptime ,' H:M:S')];
EVENT.BlockStrLength = num2str(stoptime - starttime);
...
EVENT.CORINF = Corr_INF;
%save trial list in EVENT structure
Idx = ~isnan(Trilist(:,1)); %get rid of trials without a stim onset

Trials.(Names{1}) = Trilist(Idx,1);
Trials.(Names{2}) = Trilist(Idx,2);


if ~isempty(Sacc_INF)
    Trials.(Names{3}) = Trilist(Idx,3);
end
if ~isempty(Targ_INF)
    Trials.(Names{4}) = Trilist(Idx,4);
end
if ~isempty(Corr_INF)
    Trials.(Names{5}) = Trilist(Idx,5);
end
if ~isempty(Rewd_INF)
    Trials.(Names{6}) = Trilist(Idx,6);
end
if ~isempty(Err_INF)
    Trials.(Names{7}) = Trilist(Idx,7);
end
if ~isempty(Micr_INF)
    Trials.(Names{8}) = Trilist(Idx,8);
end
if ~isempty(Word_INF)
    Trials.(Names{9}) = Trilist(Idx,9);
end

IxE = find(isnan(Trilist(Idx,9)) == 1);
if ~isempty(IxE)
    % IxW = find(isnan(Trilist(:,8)) == 0);  %select only those trials with a valid word info
    % Trilist = Trilist(IxW,:);
     errordlg(['Trials without wordinfo!!!! : ' num2str(IxE.')] )
